...
def filter_new_trades(new_trades, pos_statement_file):
    ...
    new_trades['Exec Date'] = pd.to_datetime(new_trades['Exec Time'], format='%m/%d/%y %H:%M:%S').dt.normalize()
...
def main():
    ...
    for i in range(len(acc_files)):
        ...
        if i == 0:
            first_trades = get_new_trades(acc_filepath)
            first_trades.drop(8, inplace=True)
            first_trades.to_csv('undervalued_trades.csv', index=False)
            print(f'Created "undervalued_trades.csv" using "{acc_file_name}"')
        else:
            pos_filepath = pos_files[i - 1]
            pos_file_name = os.path.basename(pos_filepath)
            new_trades = get_new_trades(acc_filepath)
            previous_trades, new_undervalued_trades = filter_new_trades(new_trades, pos_filepath)
            ...
            print('Updated "undervalued_trades.csv"')
